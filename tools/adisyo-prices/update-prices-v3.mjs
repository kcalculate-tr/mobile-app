#!/usr/bin/env node
/**
 * KCAL → Adisyo Toplu Fiyat Güncelleme (v3)
 *
 * Düzeltmeler v2'ye göre:
 *   - GetProductViewModel response'u { product: {...}, allUnits, ... } şeklinde
 *     wrapper'lı geliyor; payload.product içinden alıyoruz
 *   - SaveProduct'a göndermeden önce oldCategoryId field'ını ekliyoruz
 *     (Adisyo bunu kategoriyi değiştirmediğinden emin olmak için kullanıyor)
 *
 * Çalıştırma:
 *   node update-prices-v3.mjs --dry-run
 *   node update-prices-v3.mjs --only KCAL-7
 *   node update-prices-v3.mjs
 */
...
async function getProductViewModel(productId) {
  ...
  if (!json?.product) {
    throw new Error(`GetProductViewModel response'unda 'product' field yok. Top-level: ${Object.keys(json || {}).join(', ')}`);
  }
  return json.product;
}
...
async function saveProduct(payload) {
  // Adisyo SaveProduct, kategoriyi değiştirmediğimizi anlaması için
  // oldCategoryId field'ını bekliyor (önceki cURL'lerden öğrendik)
