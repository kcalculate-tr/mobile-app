...
import { CategoryProduct, fetchProductsByBrand } from '../lib/categories';
import { Brand, fetchBrand } from '../lib/brands';
...
export default function BrandLandingScreen() {
  const navigation = useNavigation<BrandLandingNavigationProp>();
  const route = useRoute<BrandLandingRouteProp>();
  const insets = useSafeAreaInsets();
  const { brandKey } = route.params;
  ...
  const loadProducts = useCallback(async () => {
    ...
    try {
      const [brandData, data] = await Promise.all([
        fetchBrand(brandKey),
        fetchProductsByBrand(brandKey),
      ]);
      setBrand(brandData);
      setProducts(data);
    } catch (error: unknown) {
    ...
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          message="Bu markada listelenecek ürün yok."
