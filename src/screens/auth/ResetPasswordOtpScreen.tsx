...
type RouteParams = { ResetPasswordOtp: { email: string } };

export default function ResetPasswordOtpScreen() {
  const nav = useNavigation();
  const route = useRoute<RouteProp<RouteParams, 'ResetPasswordOtp'>>();
  ...
  const handleComplete = async (entered: string) => {
    ...
    try {
      ...
      if (err) throw err;
      nav.navigate('NewPassword' as never);
    } catch {
      setError(true);
    } finally {
      setLoading(false);
    ...
    <BackgroundLayer mode="blur">
      <SafeAreaView style={styles.safe}>
        ...
        <View style={styles.content}>
          <Text style={styles.h1}>Sıfırlama kodu.</Text>
